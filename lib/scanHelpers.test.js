...
describe('CATEGORY_TAG_MAP — new "eggs" swap category', () => {
  // Eggs never had their own swap category — shell-egg-carton products fell
  // through to null (no CATEGORY_TAG_MAP entry matched, even though the
  // separate MEAT_CATEGORIES set includes en:eggs for isMeat purposes).
  // Confirmed real via the null-category audit (see
  // scripts/output/l2-v42-null-category-audit.md) and re-verified directly
  // against OFF immediately before writing these tests.

  test('a product carrying only en:eggs (no other category tag) now maps to product_category: eggs', () => {
    expect(mapProductCategory(['en:eggs'])).toBe('eggs');
  });

  test('real repro shape — Eggland\'s Best 12 Grade AA Eggs Extra Large (715141729283)\'s actual tag set maps to eggs', () => {
    // en:farming-products is a real tag on this product too, deliberately
    // NOT added to CATEGORY_TAG_MAP.eggs (too generic — would false-match
    // unrelated farm products) — confirmed the match still succeeds via the
    // other two tags regardless.
    expect(mapProductCategory([
  ...
  test('real repro shape — Free Range Eggs (815652004180)\'s actual tag set (the fullest real combination found, including both casing-variant-adjacent tags) maps to eggs', () => {
    expect(mapProductCategory([
      'en:eggs-and-their-products',
      'en:farming-products',
      'en:eggs',
  ...
  test('regression guard — a product with no egg tag at all is still unaffected (returns its real category, or null)', () => {
    expect(mapProductCategory(['en:snacks'])).toBe('snacks');
    expect(mapProductCategory(['en:not-a-real-tag'])).toBe(null);
  });

  test('regression guard — en:meats (a real MEAT_CATEGORIES/CATEGORY_TAG_MAP.meat tag) still maps to meat, not eggs — the two lists remain independent', () => {
    expect(mapProductCategory(['en:meats'])).toBe('meat');
  });
});
