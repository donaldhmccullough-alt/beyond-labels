...
/**
 * Parity test for the MEAT_CATEGORIES fix (en:eggs/en:egg-products/
 * en:poultry-eggs removed — see lib/scanHelpers.js and CLAUDE.md's "Node 8
 * vs Node 8b ordering for en:eggs" investigation). computeCorrectedVerdict()
 * imports isMeatProduct() from the same lib/scanHelpers.js module scan.js
 * uses, so this requires no code change in this file — this test exists
 * purely to confirm that shared-source fix, since verdictEngine.js has its
 * own isConventionalMeat pattern that could in principle have drifted from
 * scan.js's if the two ever stopped sharing the same underlying helper.
 */
describe('MEAT_CATEGORIES fix parity — en:eggs no longer counts as "meat" in the corrected engine either', () => {
  test('a pure egg product (categoriesTags: en:eggs only) has isMeat: false and does not get an injected conventional_meat flag', () => {
    const result = computeCorrectedVerdict({
      ingredientText: 'eggs',
      productLabels:  [],
      categoriesTags: ['en:eggs-and-their-products', 'en:eggs'],
      productName:    "Eggland's Best 12 Grade AA Eggs Extra Large",
      userLevel:      2,
    });
    expect(result.isMeat).toBe(false);
    expect(result.flags.some(f => f.category === 'conventional_meat')).toBe(false);
    expect(result.flags.some(f => f.category === 'conventional_eggs')).toBe(true);
  });

  test('regression guard — a real meat category tag (en:beef) still correctly produces isMeat: true', () => {
    const result = computeCorrectedVerdict({
      ingredientText: 'beef, salt, water.',
      productLabels:  [],
      categoriesTags: ['en:beef'],
      productName:    'Test Beef Product',
      userLevel:      2,
    });
    expect(result.isMeat).toBe(true);
    expect(result.flags.some(f => f.category === 'conventional_meat')).toBe(true);
  });
});
