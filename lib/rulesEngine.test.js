...
describe('77. stripAllergenAdvisory() — "manufactured/made/processed in a facility that (also) processes X" false-positive fix', () => {
  // Real-world reproductions, all pulled directly from scan_cache (user_level=2,
  // prompt_version=42) via a read-only audit — see CLAUDE.md for the investigation.

  test('exact reproduction — Protein Oatmeal (810104462914): "Manufactured in a facility that also processes egg..." no longer flags conventional_eggs', () => {
    const ingredients =
      'Gluten-free oats, milk protein isolate, allulose, flaxseed, chia seeds, inulin, salt, dried apples, ' +
      'natural flavors, cinnamon, apple powder, monk fruit extract, stevia extract. Contains: milk. ' +
      'Manufactured in a facility that also processes egg, soy, sesame, and tree nuts.';
    const result = analyzeIngredients(ingredients, [], 2);
    expect(result.flags.some(f => f.category === 'conventional_eggs')).toBe(false);
    // The real, legitimate "milk protein isolate" ingredient wasn't
    // collaterally stripped by the new pattern — it's not part of the
    // matched clause and sits well before it in the text. conventional_dairy
    // itself is a scan.js-level injected flag (via containsMilkDerived()),
    // not something analyzeIngredients() emits directly, so this checks the
    // underlying signal directly instead.
    expect(result.unverifiedIngredients.some(t => /milk protein isolate/i.test(t))).toBe(false);
    expect(result.flags.some(f => f.category === 'glyphosate_heavy' && f.matchedIngredient === 'oats')).toBe(true);
  });

  test('exact reproduction — Pure Protein Galactic Brownie (749826000497): "Made in a facility that also processes...egg..." no longer flags conventional_eggs', () => {
    // Verbatim real scan_cache ingredients text (OFF OCR noise — "Patrolate",
    // "Dycerine", "Comstarch", "Camaube Wax", trailing "UM POCKE" — left
    // exactly as-is; the disclosure sentence itself is intact regardless).
    const ingredients =
      'Protein Blend Soy Patrolate Whey Protein Concentrate, Whey Proton Syrup Hydrolyzed Gelatin Dycerine, ' +
      'Fractionated Palm Kernel And Palm Oil, Soy Protein Isolate Matter Whey Protein Concentrate Natural ' +
      'and Artificial Plavors Includes Annatto And Turmeric Color Canola Oil Sugar Cacta Processed With ' +
      'Akai Mix Protein Concentrate Ustened Chocolate Tapioca Starch, Soy Lecithin, Comstarch Calcium ' +
      'Carbonate, Salt, Nonfat Milk, Partially Defatted Peanut Flour, Sucralose, Dextrose, Confectioners ' +
      'Glaze Disodium Phosphate, Carrageenan, Color Blend (Red 40 Lake, Yellow 5 Lake, Yellow 6 Lake, ' +
      'Blue 1 Lake, Almond Butter, Camaube Wax, Tocopherols Added To Protect Flavor Contains milk, soy, ' +
      'peanut and tree nut (almond) Ingredients. Made in a facility that also processes other tree nuts, ' +
      'egg, wheat and sesame. Contains bioengineered food ingredients. UM POCKE';
    const result = analyzeIngredients(ingredients, [], 2);
    expect(result.flags.some(f => f.category === 'conventional_eggs')).toBe(false);
    // The mandatory bioengineering disclosure still flags — confirms the new
    // pattern didn't accidentally consume it (it sits in a separate sentence).
    expect(result.flags.some(f => f.category === 'bioengineering')).toBe(true);
  });

  test('exact reproduction — Back To Nature (819898010080): "MADE IN A FACILITY THAT ALSO PROCESSES SOY, EGGS, MILK, COCONUT." (all caps, no "also"-optional needed here but confirms case-insensitivity) no longer flags conventional_eggs', () => {
    const ingredients =
      'ENRICHED WHEAT FLOUR (WHEAT FLOUR, NIACIN, REDUCED IRON, THIAMINE MONONITRATE, RIBOFLAVIN, FOLIC ACID), ' +
      'SAFFLOWER OIL, WHOLE WHEAT FLOUR, WHEAT FLOUR, GOLDEN FLAX SEED, CORNSTARCH, WHOLE GRAIN MILLET, ' +
      'POTATO STARCH, BROWN RICE SYRUP, BROWN FLAX SEED, WHOLE GRAIN TRITICALE, WHOLE GRAIN BARLEY FLAKES, ' +
      'WHOLE GRAIN RYE, OATS, CANE SUGAR, LEAVENING (AMMONIUM BICARBONATE, MONOCALCIUM PHOSPHATE, BAKING SODA), ' +
      'MALTED BARLEY EXTRACT, SEA SALT, GARLIC POWDER, PARSLEY, ONION POWDER, CITRIC ACID, MIXED TOCOPHEROLS ' +
      '(TO PRESERVE FRESHNESS). CONTAINS: WHEAT. MADE IN A FACILITY THAT ALSO PROCESSES SOY, EGGS, MILK, COCONUT.';
    const result = analyzeIngredients(ingredients, [], 2);
    expect(result.flags.some(f => f.category === 'conventional_eggs')).toBe(false);
    // Real wheat/barley/rye/flax ingredients still correctly flag — confirms
    // the new pattern didn't collaterally strip anything upstream of it.
    expect(result.flags.some(f => f.category === 'glyphosate_heavy' && f.matchedIngredient === 'wheat flour')).toBe(true);
  });

  test('"processes X" without the word "also" is also stripped (the "also" is optional in the pattern)', () => {
    const result = analyzeIngredients(
      'almonds, sea salt. Processed in a facility that processes tree nuts and eggs.',
      [], 2
    );
    expect(result.flags.some(f => f.category === 'conventional_eggs')).toBe(false);
  });

  test('exact reproduction — Nicks Birthday cake (850009682130): a genuinely unterminated (no period anywhere after the disclosure), heavily OCR-corrupted facility statement is left unstripped — a known, accepted limitation, not a crash or a runaway match', () => {
    // Verbatim real scan_cache ingredients text. Confirmed via direct
    // inspection: no "." character exists anywhere from "PROCESSED IN A
    // FACILITY" to the end of the string, so the new pattern's
    // period-terminated `[^.]*\.` simply fails to match this occurrence
    // rather than running away to some unrelated, distant period — the
    // same conservative behavior as the pre-existing "Manufactured by:"
    // pattern's own (documented) lack of an end-of-string fallback. See
    // CLAUDE.md's known-issues note on this barcode's OFF ingredients_text
    // field actually containing OCR'd nutrition-facts-panel text.
    const ingredients =
      "Facts container 13 cup (91g) Per container 90 260 % DV* 22% 20% 60% 3% 35mg 12% 360mg 16% 22% 71% 0% " +
      "*The % Daily Value (DV) tells you how much a nutrient in a serving of food contributes to a daily diet " +
      "2,000 calories a day is used for general nutrition advice  CREAM, DAIRY PROTEIN BLEND (WATER, MILK " +
      "PROTEIN CONCENTRATE, WHEY PROTEIN ISOLATE, ALLULOSE), WATER, SOLUBLE CORN FIBER, ALLULOSE, EPG " +
      "(MODIFIED PLANT-BASED OIL), ERYTHRITOL, LESS THAN 2% OF: COCONUT OIL, NATURAL FLAVOR, PALM KERNEL OIL, " +
      "STEVIOL GLYCOSIDES, FRUIT JUICE FOR COLOR, ANNATTO AND BETA CAROTENE FOR* COLOR, SALT, GUAR GUM, " +
      "CELLULOSE GUM, CELLULOSE GEL, MONO ,  DIGLYCERIDES, SUNFLOWER LECITHIN, GUM ACACIA, CARNAUBA WAX " +
      "CONTAINS: MILK, COCONUT PROCESSED IN A FACILITY THAT PROCESSES TREE NUTS AND EGGS H7 NET CARBS/PINT = " +
      "TOTAL CARB (61G) - DIETARY FIBER (20G) - SUGAR ALCOHOLS (15G) - ALLULOSE (19G) NICK'S BIRTHDÄG CAKE " +
      "LIGHT ICE CREAM CONTAINS 90 CALORIES ,  6G TOTAL FAT PER SERVING COMPARED TO 233 CALORIES ,  12G TOTAL " +
      "FAI PER SERVING FOR REGULAR BIRTHDAY CAKE ICE CREAM • IMAGES ARE ONLY SUGGESTIVE OF FLAVOR DOES " +
      "NOT CONTAIN WHOLE CAKE PIECES 15g uingy Kông 15% 160 mg 50009 68213 MANUFACTURED FOR LUB FOODS " +
      "USA, INC EL SEGUNDO, CA 90245 f #nicksicecreams wwwnickscom";
    const result = analyzeIngredients(ingredients, [], 2);
    const eggFlag = result.flags.find(f => f.category === 'conventional_eggs');
    expect(eggFlag).toBeDefined();
    expect(eggFlag.matchedIngredient).toBe('eggs');
  });

  test('regression guard — "manufactured on a line" (existing pattern) is unaffected by the new pattern', () => {
    const result = analyzeIngredients(
      'almonds, canola oil. Manufactured on a line that also processes peanuts and soy.',
      [], 2
    );
    expect(result.flags.some(f => f.category === 'seed_oils')).toBe(true);
    const nonSeedOilFlags = result.flags.filter(f => f.category !== 'seed_oils' && f.category !== 'gluten_grains');
  ...
  test('regression guard — "produced in a facility" (existing pattern) is unaffected by the new pattern', () => {
    const result = analyzeIngredients(
      'almonds, canola oil. Produced in a facility that also processes peanuts and soy.',
      [], 2
    );
    expect(result.flags.some(f => f.category === 'seed_oils')).toBe(true);
    expect(result.unverifiedIngredients).toHaveLength(0);
  ...
  test('regression guard — "Manufactured by: [company] [address]\\n" (existing pattern) is unaffected by the new pattern', () => {
    const result = analyzeIngredients(
      'salt, sugar. Manufactured by: Foo Corp, 123 Main St., Springfield, IL 62704\nolive oil',
      [], 2
    );
  ...
  test('regression guard — "THIS PRODUCT IS MADE IN A [...] FACILITY [...]." (existing pattern) is unaffected by the new pattern', () => {
    const result = analyzeIngredients(
      'salt, sugar. THIS PRODUCT IS MADE IN A SOY-FREE, PEANUT-FREE FACILITY DOES NOT PROCESS NUTS.',
      [], 2
    );
  ...
  test('regression guard — a real, standalone egg ingredient still correctly flags conventional_eggs', () => {
    const result = analyzeIngredients('wheat flour, eggs, sugar, sea salt.', [], 2);
    expect(result.flags.some(f => f.category === 'conventional_eggs' && f.matchedIngredient === 'eggs')).toBe(true);
  });
});

describe('78. containsFortifiedVitamins() — "calcium phosphate" substring collision fix', () => {
  // Real-world reproductions, all pulled directly from scan_cache via a
  // read-only audit — see CLAUDE.md for the investigation. 'calcium
  // phosphate' is a literal substring of 'monocalcium phosphate',
  // 'dicalcium phosphate', and 'tricalcium phosphate' — all real, common
  // food-grade leavening acids / dough conditioners, not an intentional
  // vitamin/mineral fortification additive.

  test('exact reproduction — Complete Pancake & Waffle Mix (810001560522): "Monocalcium Phosphate" no longer flags fortified_vitamins', () => {
    const ingredients =
      'Organic Wheat Flour, Organic Cane Sugar, Organic, organic Buttermilk, Organic Skim Milk, ' +
      'Monocalcium Phosphate, Baking Soda, Organic Pea Starch, Organic Milk Powder, Salt, ' +
      'Organic Potato Starch, Natural Flavor';
    expect(containsFortifiedVitamins(ingredients.toLowerCase())).toBe(false);
  });

  test('exact reproduction — Frosted Wildberry Acai Toaster Pastries (058449410225): "monocalcium phosphate" no longer flags fortified_vitamins', () => {
    const ingredients =
      'Wheat flour*, cane sugar*, invert cane syrup*, palm oil*, whole wheat flour*, apples*, ' +
      'raspberries*, corn starch*, flavors* (wildberry, strawberry, blueberry, raspberry), ' +
      'blueberries*, rice starch*, dextrose*, leavening (baking soda, cream of tartar), ' +
      'vital wheat gluten*, strawberries*, tapioca starch*, sea salt, color (red beet powder, ' +
      'paprika oleoresin), honey*, molasses*, citric acid, whey protein concentrate* (milk), ' +
      'acai powder*, algin, sodium citrate, monocalcium phosphate.';
    expect(containsFortifiedVitamins(ingredients.toLowerCase())).toBe(false);
  });

  test('exact reproduction — Frosted Berry Strawberry Toaster Pastries (058449410003): "monocalcium phosphate" no longer flags fortified_vitamins', () => {
    const ingredients =
      'Wheat flour*, cane sugar*, invert cane syrup*, palm oil*, whole wheat flour*, apples*, ' +
      'corn starch*, strawberries*, rice starch*, leavening (baking soda, cream of tartar), ' +
      'vital wheat gluten*, dextrose*, strawberry flavor*, sea salt, color (red beet powder, ' +
      'paprika oleoresin), tapioca starch*, honey*, molasses*, citric acid, whey protein ' +
      'concentrate* (milk), algin, sodium citrate, monocalcium phosphate.';
    expect(containsFortifiedVitamins(ingredients.toLowerCase())).toBe(false);
  });

  test('exact reproduction — Cheddar Bunnies Original (013562302154): "monocalcium phosphate" no longer flags fortified_vitamins', () => {
    const ingredients =
      'Organic wheat flour, organic expeller-pressed sunflower oil, salt, organic cheddar cheese ' +
      '(pasteurized organic milk, cheese cultures, sea salt, non-animal enzymes), baking soda, ' +
      'organic yeast, organic paprika, monocalcium phosphate, cultured organic milk, organic ' +
      'nonfat dry milk, organic annatto (for color), organic onion powder, organic celery seed ' +
      'powder, enzymes.';
    expect(containsFortifiedVitamins(ingredients.toLowerCase())).toBe(false);
  });

  test('regression guard — Organic Ricemilk (084253222303) STILL flags fortified_vitamins: genuine vitamin A/D2/B12 fortification independent of the "Tricalcium Phosphate" collision — proves the fix does not overcorrect', () => {
    const ingredients =
      'Organic Ricemilk (Water, Organic Hydrolyzed Rice), Organic Canola Oil, Organic Sunflower Oil, ' +
      'Tricalcium Phosphate, Sea Salt, Vitamin A Palmitate, Vitamin D2 (Ergocalciferol), ' +
      'Vitamin B12 (Cyanocobalamin).';
    expect(containsFortifiedVitamins(ingredients.toLowerCase())).toBe(true);
  });

  test('regression guard — a bare, standalone "calcium phosphate" (not embedded in a larger word) still correctly flags', () => {
    expect(containsFortifiedVitamins('rice, calcium phosphate, salt')).toBe(true);
  });

  test('regression guard — "dicalcium phosphate" is also correctly blocked (same collision shape, confirmed real via scan_cache)', () => {
    expect(containsFortifiedVitamins('wheat flour, dicalcium phosphate, sugar')).toBe(false);
  });

  test('regression guard — every other FORTIFIED_VITAMINS trigger (vitamin e, iron, etc.) is unaffected by the boundary guard', () => {
    expect(containsFortifiedVitamins('tocopherols (vitamin e)')).toBe(true);
    expect(containsFortifiedVitamins('reduced iron')).toBe(true);
    expect(containsFortifiedVitamins('ferrous sulfate')).toBe(true);
  });
});

// ════════════════════════════════════════════════════════════════════════════
// 79. GLYPHOSATE_HEAVY flag summary wording (Joel-voice certification-framing
//     audit, July 2026) — the per-trigger fallback summary rendered by
//     ConcernCard.getFallbackSummary() whenever the AI explanation is missing.
//     Reworded to drop the old "Look for a glyphosate-free or USDA Organic
//     certification to reduce residue exposure" near-guarantee framing in
//     favor of "can point toward lower exposure, though it's not a
//     guarantee" — matching the stance already shipped for conventional_eggs
//     and conventional_dairy. Distinct from those two categories' rework:
//     this string lives in lib/rulesEngine.js itself (the engine's own
//     per-match flag.summary), not pages/api/explain.js — a fourth surface
//     discovered during this category's audit, not part of the original
//     13-item table.
// ════════════════════════════════════════════════════════════════════════════

describe('79. GLYPHOSATE_HEAVY flag summary — certification-framing rework', () => {
  test('new wording present for a real trigger ("oats"), old phrasing gone', () => {
    const result = analyzeIngredients('oats, salt, water', [], 2);
    const flag = result.flags.find(f => f.category === 'glyphosate_heavy' && f.matchedIngredient === 'oats');
    expect(flag).toBeDefined();
    expect(flag.summary).toBe(
      'Contains "oats" — a crop where glyphosate is routinely applied as a pre-harvest desiccant. ' +
      'A glyphosate-free or USDA Organic label can point toward lower exposure, though it\'s not a guarantee.'
    );
    expect(flag.summary).not.toContain('Look for a glyphosate-free or USDA Organic certification');
    expect(flag.summary).not.toContain('to reduce residue exposure');
  });

  test('new wording is per-trigger (dynamic) — confirmed with a second real trigger ("wheat")', () => {
    const result = analyzeIngredients('wheat, salt, water', [], 2);
    const flag = result.flags.find(f => f.category === 'glyphosate_heavy' && f.matchedIngredient === 'wheat');
    expect(flag).toBeDefined();
    expect(flag.summary).toBe(
      'Contains "wheat" — a crop where glyphosate is routinely applied as a pre-harvest desiccant. ' +
      'A glyphosate-free or USDA Organic label can point toward lower exposure, though it\'s not a guarantee.'
    );
  });

  test('regression guard — severity and verdict are unaffected by the wording change (still reject/red with no clearance)', () => {
    const result = analyzeIngredients('oats, salt, water', [], 2);
    const flag = result.flags.find(f => f.category === 'glyphosate_heavy');
    expect(flag.severity).toBe('reject');
    expect(result.verdict).toBe('red');
...
// ════════════════════════════════════════════════════════════════════════════
// 80. TRANS_FATS flag summary — bare "hydrogenated" hedge, PLUS the qualified-
// branch ("partially hydrogenated"/"margarine"/"shortening") GRAS-reversal
// reword added in a later session (PROMPT_VERSION 44→45) — see CLAUDE.md's
// Joel-voice-adjacent trans_fats fix entries for both. The bare-"hydrogenated"
// wording (tested first below) is unchanged from when it was originally
// approved: since the FDA's partially-hydrogenated-oil ban completed in 2023
// with no remaining exceptions, bare "hydrogenated" with no qualifier most
// often means fully hydrogenated oil today — not a meaningful trans fat
// source. The qualified-branch triggers were originally left with their
// pre-existing "directly linked to cardiovascular disease" wording — that
// wording is now replaced with the same GRAS-reversal framing approved for
// the SYSTEM_PROMPT's dedicated trans_fats paragraph (the FDA revoked GRAS
// status in 2015 and banned the ingredient outright by 2018 — regulators
// acting decisively, not lax oversight), kept short since this is a fallback
// string (shown by ConcernCard.getFallbackSummary() only when the AI
// explanation is missing), not the full narrative explanation.
// ════════════════════════════════════════════════════════════════════════════

describe('80. TRANS_FATS flag summary — bare "hydrogenated" hedge + qualified-branch GRAS-reversal reword', () => {
  test('bare "hydrogenated" gets the new hedged single-sentence summary', () => {
    const result = analyzeIngredients('peanuts, hydrogenated rapeseed oil, salt', [], 2);
    const flag = result.flags.find(f => f.category === 'trans_fats' && f.matchedIngredient === 'hydrogenated');
    expect(flag).toBeDefined();
    expect(flag.summary).toBe(
      'Contains "hydrogenated" oil — since the FDA\'s ban on partially hydrogenated ' +
      'oils is now complete with no exceptions, this most often means fully ' +
      'hydrogenated oil (not a meaningful trans fat source), though it\'s flagged ' +
      'to be safe given the possibility of imported or mislabeled products.'
    );
    expect(flag.summary).not.toContain('directly linked');
    expect(flag.summary).not.toContain('cardiovascular disease');
  });

  test('"partially hydrogenated" gets the new GRAS-reversal fallback summary', () => {
    const result = analyzeIngredients('vegetable shortening (partially hydrogenated soybean oil)', [], 2);
    const flag = result.flags.find(f => f.category === 'trans_fats' && f.matchedIngredient === 'partially hydrogenated');
    expect(flag).toBeDefined();
    expect(flag.summary).toBe(
      'Contains "partially hydrogenated" — the primary source of artificial trans fats, though the ' +
      'FDA revoked its GRAS status in 2015 and banned it outright by 2018, so this likely ' +
      'reflects an outdated formulation or import rather than a current gap in oversight.'
    );
    expect(flag.summary).not.toContain('directly linked');
    expect(flag.summary).not.toContain('cardiovascular disease');
  });

  test('"margarine" and "shortening" both get the new GRAS-reversal fallback summary', () => {
    const margarineResult = analyzeIngredients('flour, margarine, sugar', [], 2);
    const margarineFlag = margarineResult.flags.find(f => f.category === 'trans_fats' && f.matchedIngredient === 'margarine');
    expect(margarineFlag.summary).toBe(
      'Contains "margarine" — the primary source of artificial trans fats, though the ' +
      'FDA revoked its GRAS status in 2015 and banned it outright by 2018, so this likely ' +
      'reflects an outdated formulation or import rather than a current gap in oversight.'
    );

    const shorteningResult = analyzeIngredients('flour, shortening, sugar', [], 2);
    const shorteningFlag = shorteningResult.flags.find(f => f.category === 'trans_fats' && f.matchedIngredient === 'shortening');
    expect(shorteningFlag.summary).toBe(
      'Contains "shortening" — the primary source of artificial trans fats, though the ' +
      'FDA revoked its GRAS status in 2015 and banned it outright by 2018, so this likely ' +
      'reflects an outdated formulation or import rather than a current gap in oversight.'
    );
  });

  test('regression guard — severity and verdict are unaffected by the wording change (still reject/red, both triggers)', () => {
    const bareResult = analyzeIngredients('peanuts, hydrogenated rapeseed oil, salt', [], 2);
    const bareFlag = bareResult.flags.find(f => f.category === 'trans_fats');
    expect(bareFlag.severity).toBe('reject');
    expect(bareResult.verdict).toBe('red');

    const partialResult = analyzeIngredients('vegetable shortening (partially hydrogenated soybean oil)', [], 2);
    const partialFlag = partialResult.flags.find(f => f.category === 'trans_fats');
    expect(partialFlag.severity).toBe('reject');
    expect(partialResult.verdict).toBe('red');
  });
});

describe('81. ALL_TRIGGERS — NATURAL_COLORANTS suppression from unverifiedIngredients (bug fix)', () => {
  // NATURAL_COLORANTS (added in block 29) was never spliced into ALL_TRIGGERS —
  // the array Pass 1 of unverified-ingredient filtering actually checks against
  // (see the ALL_TRIGGERS composition just above module.exports). Every other
  // trigger-list group (SYNTHETIC_ADDITIVES, GLYPHOSATE_HEAVY, FORTIFIED_VITAMINS,
  // etc.) was already spread into it — NATURAL_COLORANTS was the sole omission.
  // This meant already-recognized colorants (annatto, paprika extract, oleoresin
  // of paprika, ...) incorrectly surfaced as "unrecognized" in unverifiedIngredients
  // even though containsNaturalColorants() already has rules for every one of them
  // (block 29). Display-only — Pass 1 of the unverified-token filter runs entirely
  // independently of flags/verdict calculation (containsNaturalColorants() is only
  // ever consulted separately, in the L2 organic-path branch), so none of these
  // tests should observe any change to flags or verdict. Mirrors the pattern used
  // for the identical MEAT_INGREDIENT_TERMS/MEAT_DERIVED_INGREDIENTS/
  // MILK_DERIVED_INGREDIENTS fix in block 76.

  test.each(NATURAL_COLORANTS)('"%s" no longer appears in unverifiedIngredients', (colorant) => {
    const result = analyzeIngredients(`${colorant}, water, sea salt`, [], 2);
    expect(result.unverifiedIngredients).not.toContain(colorant);
  });

  test('regression: flags and verdict are completely unaffected by the "annatto" fixture — this is a display-only Pass 1 change', () => {
    const withColorant = analyzeIngredients('cheddar cheese, annatto, sea salt', [], 2);
    const withoutColorant = analyzeIngredients('cheddar cheese, sea salt', [], 2);
    expect(withColorant.flags.map(f => f.category).sort()).toEqual(withoutColorant.flags.map(f => f.category).sort());
    expect(withColorant.verdict).toBe(withoutColorant.verdict);
  });

  test('regression: flags and verdict are completely unaffected by the "oleoresin of paprika" fixture', () => {
    const withColorant = analyzeIngredients('chips, oleoresin of paprika, sunflower oil', [], 2);
    const withoutColorant = analyzeIngredients('chips, sunflower oil', [], 2);
    expect(withColorant.flags.map(f => f.category).sort()).toEqual(withoutColorant.flags.map(f => f.category).sort());
    expect(withColorant.verdict).toBe(withoutColorant.verdict);
  });

  test('regression: flags and verdict are completely unaffected by the "beta-carotene" fixture', () => {
    const withColorant = analyzeIngredients('margarine, beta-carotene, palm oil', [], 2);
    const withoutColorant = analyzeIngredients('margarine, palm oil', [], 2);
    expect(withColorant.flags.map(f => f.category).sort()).toEqual(withoutColorant.flags.map(f => f.category).sort());
    expect(withColorant.verdict).toBe(withoutColorant.verdict);
  });

  test('regression: containsNaturalColorants() (block 29) is completely unaffected — still detects the same colorants via its own independent check', () => {
    expect(containsNaturalColorants('cheddar cheese, annatto, salt')).toBe(true);
    expect(containsNaturalColorants('chips, paprika extract, sunflower oil')).toBe(true);
    expect(containsNaturalColorants('organic oats, honey, sea salt')).toBe(false);
  });

  test('regression: an ingredient genuinely not on any list still appears in unverifiedIngredients (Pass 1 is not now suppressing everything)', () => {
    const result = analyzeIngredients('mystery extract, sea salt', [], 2);
    expect(result.unverifiedIngredients).toContain('mystery extract');
  });
});

describe('82. SYNTHETIC_ADDITIVES — sodium acid pyrophosphate, methylcellulose (approved additions)', () => {
  // Both entries follow the exact same mechanism as every other SYNTHETIC_ADDITIVES
  // trigger (see block 26's "GROUP B — Synthetic phosphates" for the precedent this
  // mirrors): severity is hardcoded 'reject' in the matching loop, category is
  // always 'additives', and 'additives' is an INSTANT_RED_CATEGORIES member in
  // scan.js — so a match forces verdict 'red' unconditionally at both L1 and L2,
  // with no organic/certification exception. This is the confirmed, approved
  // outcome for both entries — not a gap to be fixed later.

  test('"sodium acid pyrophosphate" triggers a synthetic_additives reject flag at Level 2', () => {
    const result = analyzeIngredients('water, sodium acid pyrophosphate, salt', [], 2);
    const flag = flagsFor(result, 'additives')[0];
    expect(flag).toBeDefined();
    expect(flag.severity).toBe('reject');
    expect(flag.matchedIngredient).toBe('sodium acid pyrophosphate');
    expect(result.verdict).toBe('red');
  });

  test('"sodium acid pyrophosphate" — red at Level 1 too (additives is not level-dependent)', () => {
    expect(analyzeIngredients('water, sodium acid pyrophosphate, salt', [], 1).verdict).toBe('red');
  });

  test('"sodium acid pyrophosphate" — case-insensitive match', () => {
    const result = analyzeIngredients('water, Sodium Acid Pyrophosphate, salt', [], 2);
    expect(flagsFor(result, 'additives').find(f => f.matchedIngredient === 'sodium acid pyrophosphate')).toBeDefined();
    expect(result.verdict).toBe('red');
  });

  test('"sodium acid pyrophosphate" is not shadowed by or confused with "tetrasodium diphosphate"/"tetrasodium pyrophosphate"/"sodium hexametaphosphate"/"sodium aluminum phosphate" — each still matches its own distinct trigger', () => {
    const result = analyzeIngredients(
      'tetrasodium diphosphate, tetrasodium pyrophosphate, sodium acid pyrophosphate, sodium hexametaphosphate, sodium aluminum phosphate',
      [], 2
    );
    const matched = flagsFor(result, 'additives').map(f => f.matchedIngredient).sort();
  ...
  test('"methylcellulose" triggers a synthetic_additives reject flag at Level 2', () => {
    const result = analyzeIngredients('water, methylcellulose, salt', [], 2);
    const flag = flagsFor(result, 'additives')[0];
    expect(flag).toBeDefined();
    expect(flag.severity).toBe('reject');
    expect(flag.matchedIngredient).toBe('methylcellulose');
    expect(result.verdict).toBe('red');
  });

  test('"methylcellulose" — red at Level 1 too (additives is not level-dependent)', () => {
    expect(analyzeIngredients('water, methylcellulose, salt', [], 1).verdict).toBe('red');
  });

  test('"methylcellulose" — case-insensitive match', () => {
    const result = analyzeIngredients('water, Methylcellulose, salt', [], 2);
    expect(flagsFor(result, 'additives').find(f => f.matchedIngredient === 'methylcellulose')).toBeDefined();
    expect(result.verdict).toBe('red');
  });

  test('regression: "modified cellulose" (pre-existing entry) still matches its own distinct trigger, unaffected by the new "methylcellulose" entry', () => {
    const result = analyzeIngredients('water, modified cellulose, salt', [], 2);
    expect(flagsFor(result, 'additives').find(f => f.matchedIngredient === 'modified cellulose')).toBeDefined();
    expect(result.verdict).toBe('red');
  });

  test('"methylcellulose" is unaffected by organic certification — no clearance path exists for additives', () => {
    const result = analyzeIngredients('water, methylcellulose, salt', ['usda-organic'], 2);
    expect(flagsFor(result, 'additives').find(f => f.matchedIngredient === 'methylcellulose')).toBeDefined();
    expect(result.verdict).toBe('red');
  });
});

describe('83. FORTIFIED_VITAMINS — sodium ascorbate (approved addition, organic-gated caution)', () => {
  // Mirrors block 45's exact pattern for ferric phosphate: FORTIFIED_VITAMINS
  // entries are only ever consulted via the containsFortifiedVitamins() boolean
  // helper — analyzeIngredients() itself never creates a 'fortified_vitamins'
  // flag directly (that flag is only ever assigned in pages/api/scan.js /
  // lib/verdictEngine.js, gated behind `if (hasOrganic)`). So "organic-gated
  // caution flag" behavior is confirmed at this layer via the boolean detector;
  // the flag-creation code path itself is exercised in __tests__/api/scan.test.js.

  test('"sodium ascorbate" is detected by containsFortifiedVitamins()', () => {
    expect(containsFortifiedVitamins('rice flour, sodium ascorbate, sea salt')).toBe(true);
  });

  test('"sodium ascorbate" — case-insensitive detection', () => {
    expect(containsFortifiedVitamins('Rice Flour, Sodium Ascorbate, Sea Salt')).toBe(true);
  });

  test('regression: "ascorbic acid" (a separate, pre-existing CONVENTIONAL_CROPS trigger) is unaffected by the new "sodium ascorbate" FORTIFIED_VITAMINS entry', () => {
    const result = analyzeIngredients('water, ascorbic acid, salt', [], 2);
    expect(result.flags.some(f => f.category === 'conventional_crops' && f.matchedIngredient === 'ascorbic acid')).toBe(true);
    expect(containsFortifiedVitamins('water, ascorbic acid, salt')).toBe(false);
  });

  test('"sodium ascorbate" alone produces no flags/verdict change via analyzeIngredients() — the fortified_vitamins flag is only ever created in scan.js/verdictEngine.js, gated on organic certification, not here', () => {
    const result = analyzeIngredients('water, sodium ascorbate, salt', [], 2);
    expect(result.flags.some(f => f.category === 'fortified_vitamins')).toBe(false);
    expect(result.verdict).toBe('green');
  });

  test('"sodium ascorbate" is suppressed from unverifiedIngredients (FORTIFIED_VITAMINS is spliced into ALL_TRIGGERS)', () => {
    const result = analyzeIngredients('water, sodium ascorbate, salt', [], 2);
    expect(result.unverifiedIngredients).not.toContain('sodium ascorbate');
  });
});

describe('84. ALWAYS_IGNORE_INGREDIENTS — potassium chloride, coffee (approved additions)', () => {
  // Confirmed low-concern, whitelist-only additions. ALWAYS_IGNORE_INGREDIENTS
  // entries never create a flag of any kind (unlike SYNTHETIC_ADDITIVES/
  // FORTIFIED_VITAMINS) — they are checked as a second, independent condition
  // right below ALL_TRIGGERS in the unverified-token Pass 1 filter (see the
  // filter at the bottom of analyzeIngredients()), so membership here purely
  // suppresses the token from unverifiedIngredients. Mirrors the membership/
  // ordering assertion style used for this list in block 32, plus the
  // unverifiedIngredients/flags/verdict assertion style used to verify
  // sodium ascorbate's organic-only gate in block 83.

  // ── Membership ────────────────────────────────────────────────────────────

  test('ALWAYS_IGNORE_INGREDIENTS includes "potassium chloride"', () => {
    expect(ALWAYS_IGNORE_INGREDIENTS).toContain('potassium chloride');
  });

  test('ALWAYS_IGNORE_INGREDIENTS includes "coffee"', () => {
    expect(ALWAYS_IGNORE_INGREDIENTS).toContain('coffee');
  });

  // ── "potassium chloride" — no longer unverified, no flag, no verdict change ──

  test('"potassium chloride" no longer appears in unverifiedIngredients', () => {
    const result = analyzeIngredients('water, potassium chloride, salt', [], 2);
    expect(result.unverifiedIngredients).not.toContain('potassium chloride');
  });

  test('"potassium chloride" — case-insensitive suppression', () => {
    const result = analyzeIngredients('water, Potassium Chloride, salt', [], 2);
    expect(result.unverifiedIngredients).not.toContain('potassium chloride');
  });

  test('a product containing only "potassium chloride" produces no flags and resolves green', () => {
    const result = analyzeIngredients('water, potassium chloride, salt', [], 2);
    expect(result.flags).toHaveLength(0);
    expect(result.verdict).toBe('green');
  });

  // ── "coffee" — no longer unverified, no flag, no verdict change ──────────

  test('"coffee" no longer appears in unverifiedIngredients', () => {
    const result = analyzeIngredients('coffee, water, sea salt', [], 2);
    expect(result.unverifiedIngredients).not.toContain('coffee');
  });

  test('"coffee" — case-insensitive suppression', () => {
    const result = analyzeIngredients('Coffee, water, sea salt', [], 2);
    expect(result.unverifiedIngredients).not.toContain('coffee');
  });

  test('a product containing only "coffee" produces no flags and resolves green', () => {
    const result = analyzeIngredients('coffee, water, sea salt', [], 2);
    expect(result.flags).toHaveLength(0);
    expect(result.verdict).toBe('green');
  });

  // ── Regression: pre-existing minerals in the same section are unaffected ──

  test('regression: "calcium carbonate" and "magnesium oxide" (pre-existing entries) are unaffected by the new "potassium chloride" entry', () => {
    const result = analyzeIngredients('calcium carbonate, magnesium oxide, potassium chloride, water', [], 2);
    expect(result.unverifiedIngredients).not.toContain('calcium carbonate');
    expect(result.unverifiedIngredients).not.toContain('magnesium oxide');
    expect(result.unverifiedIngredients).not.toContain('potassium chloride');
    expect(result.flags).toHaveLength(0);
    expect(result.verdict).toBe('green');
  });

  test('regression: an ingredient genuinely not on any list still appears in unverifiedIngredients', () => {
    const result = analyzeIngredients('mystery extract, sea salt', [], 2);
    expect(result.unverifiedIngredients).toContain('mystery extract');
  });
});
