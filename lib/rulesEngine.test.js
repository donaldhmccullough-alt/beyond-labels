...
describe('12. filterUnrecognizedTokens — unverifiedIngredients heuristics', () => {
  ...
  test('artifact phrase "less than" is suppressed at both levels', () => {
    const r = analyzeIngredients('water, less than, sea salt', [], 2);
    expect(r.unverifiedIngredients.map(t => t.toLowerCase())).not.toContain('less than');
  });

  test('single-character token is suppressed (< 3 chars)', () => {
    // parseIngredientTokens already filters length <= 1, but 2-char tokens
    // ("or") should also be suppressed by the < 3 rule.
    const r = analyzeIngredients('water, or, salt', [], 2);
    expect(r.unverifiedIngredients.map(t => t.toLowerCase())).not.toContain('or');
  });

  test('purely numeric token is suppressed', () => {
    // "2%" can appear as a label artifact
    const r = analyzeIngredients('salt, 2%, water', [], 2);
    expect(r.unverifiedIngredients.map(t => t.toLowerCase())).not.toContain('2%');
  });

  // ── Level 1 — whole-food words ────────────────────────────────────────────

  test('"water" is suppressed at Level 1', () => {
    const r = analyzeIngredients('water, turmeric', [], 1);
    expect(r.unverifiedIngredients.map(t => t.toLowerCase())).not.toContain('water');
  });

  test('"salt" is suppressed at Level 1', () => {
    const r = analyzeIngredients('water, salt, turmeric', [], 1);
    expect(r.unverifiedIngredients.map(t => t.toLowerCase())).not.toContain('salt');
  });

  test('"honey" is suppressed at Level 1', () => {
    const r = analyzeIngredients('honey, water', [], 1);
    expect(r.unverifiedIngredients.map(t => t.toLowerCase())).not.toContain('honey');
  });

  test('"egg" and "eggs" are suppressed at Level 1', () => {
    const r = analyzeIngredients('eggs, egg, water', [], 1);
    const ui = r.unverifiedIngredients.map(t => t.toLowerCase());
    expect(ui).not.toContain('egg');
    expect(ui).not.toContain('eggs');
  });

  test('"milk", "cream", "butter" suppressed at Level 1', () => {
    const r = analyzeIngredients('milk, cream, butter, water', [], 1);
    const ui = r.unverifiedIngredients.map(t => t.toLowerCase());
  ...
  test('"flour" and "sugar" suppressed at Level 1', () => {
    const r = analyzeIngredients('flour, sugar, water', [], 1);
    const ui = r.unverifiedIngredients.map(t => t.toLowerCase());
    expect(ui).not.toContain('flour');
    expect(ui).not.toContain('sugar');
  });

  // ── Level 1 — Level-1-yellow trigger tokens ───────────────────────────────

  test('token containing a SEED_OILS trigger is suppressed at Level 1', () => {
    // "canola oil" is in SEED_OILS; ALL_TRIGGERS filter removes it first,
    // so it never reaches unverifiedIngredients regardless.
  ...
  // ── Level 2 — plain "salt" surfaces (not in L2 list) ─────────────────────

  test('"salt" (plain) surfaces as unrecognized at Level 2', () => {
    const r = analyzeIngredients('water, salt, turmeric', [], 2);
    // turmeric and plain salt both surface at L2
    const ui = r.unverifiedIngredients.map(t => t.toLowerCase());
    expect(ui).toContain('salt');
  });

  // ── Real-world surface tokens ─────────────────────────────────────────────

  test('genuinely unknown ingredient surfaces at Level 2', () => {
    const r = analyzeIngredients('water, turmeric, sea salt', [], 2);
    // turmeric is not in any trigger or L2 suppression list → should surface
    expect(r.unverifiedIngredients.map(t => t.toLowerCase())).toContain('turmeric');
  });

  test('genuinely unknown ingredient surfaces at Level 1', () => {
    const r = analyzeIngredients('water, turmeric, salt', [], 1);
    // water and salt are suppressed; turmeric surfaces
    const ui = r.unverifiedIngredients.map(t => t.toLowerCase());
