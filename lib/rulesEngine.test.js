...
// ════════════════════════════════════════════════════════════════════════════
// 80. TRANS_FATS flag summary — bare "hydrogenated" hedge (wording approved as
// final — see CLAUDE.md changelog entry for this session's trans_fats
// explanation fix. Same pattern as block 79's glyphosate_heavy rework: the
// FDA's partially-hydrogenated-oil ban completed in 2023 with no remaining
// exceptions, so bare "hydrogenated" with no qualifier most often means fully
// hydrogenated oil today — not a meaningful trans fat source. Unlike block
// 79, only ONE trigger ("hydrogenated") gets new wording; "partially
// hydrogenated"/"margarine"/"shortening" keep the original, unhedged summary
// since those remain unambiguous real trans fat sources.
// ════════════════════════════════════════════════════════════════════════════

describe('80. TRANS_FATS flag summary — bare "hydrogenated" hedge', () => {
  ...
  test('regression guard — "partially hydrogenated" keeps the original, unhedged summary', () => {
    ...
    expect(flag).toBeDefined();
    expect(flag.summary).toBe(
      'Contains "partially hydrogenated" — a trans fat or hydrogenated oil directly linked ' +
      'to cardiovascular disease and systemic inflammation.'
    );
  });

  test('regression guard — "margarine" and "shortening" both keep the original, unhedged summary', () => {
    ...
    expect(margarineFlag.summary).toBe(
      'Contains "margarine" — a trans fat or hydrogenated oil directly linked ' +
      'to cardiovascular disease and systemic inflammation.'
    );
    ...
    expect(shorteningFlag.summary).toBe(
      'Contains "shortening" — a trans fat or hydrogenated oil directly linked ' +
      'to cardiovascular disease and systemic inflammation.'
    );
  });
