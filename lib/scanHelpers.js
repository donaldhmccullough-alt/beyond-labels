...
const MEAT_CATEGORIES = new Set([
  ...
  // 'en:eggs', 'en:egg-products', 'en:poultry-eggs' were removed here (see
  // CLAUDE.md's "Node 8 vs Node 8b ordering for en:eggs" investigation).
  // They were bundled into this set on May 21 2026 (commit e394a6a), before
  // conventional_eggs existed as its own category — the docstring below
  // used to read "matches a known meat/fish/egg category" for that reason.
  // Once conventional_eggs became a real, dedicated ingredient-text-based
  // detection mechanism (commit 9058727, 11 days later), these tags became
  // a pure vestige: a pure egg carton (Eggland's Best, Free Range Eggs,
  // etc.) tagged en:eggs in OFF would make isMeat true via isMeatCategory,
  // causing Phase A's conventional_meat injection (and the mirrored L1
  // Override 2, and lib/verdictEngine.js's parallel implementation, and
  // VerdictScreen's getUnverifiedCopy() meat-specific messaging) to fire
  // alongside the engine's own accurate conventional_eggs flag — a real,
  // confirmed duplicate/miscategorization bug, not a legitimate dependency.
  // Confirmed nothing else legitimately depends on eggs counting as "meat":
  // the swaps system's CATEGORY_TAG_MAP is a separate, unrelated constant
  // with no egg tags; seafood/game-meat detection use their own separate
  // Sets. isMeatIngredient (real ingredient-text corroboration via
  // MEAT_INGREDIENT_TERMS, which already excludes eggs — that's
  // CONVENTIONAL_EGGS/EGG_DERIVED_INGREDIENTS's job) is untouched by this
  // change, so a genuine meat+egg composite product (e.g. a turkey sausage
  // and egg breakfast bowl) still correctly gets conventional_meat via that
  // independent signal.
]);

/**
 * Returns true if any OFF categories_tags value matches a known meat/fish category.
 *
