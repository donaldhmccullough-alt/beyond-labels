...
describe('D. Level 2 behavior', () => {
  ...
  test('CORRECTION #1 — bioengineering row diverges from live scan.js and gains organic-label + organic-prefix clearance (NOT non-gmo)', () => {
    ...
    const prefixClearance = rule.clearance.find(c => c.mechanism === 'organic-ingredient-prefix');
    expect(prefixClearance).toBeDefined();
    expect(prefixClearance.effect).toBe('full-clearance');
  ...
  test('REVISED (post-Stage-4) — bioengineering is NOT cleared by non-gmo-project-verified label', () => {
    const rule = ruleFor('bioengineering');
  ...
  test('REVISED (post-Stage-4) — the design decisions log carries the organic-cert-vs-private-cert reasoning for excluding non-gmo clearance', () => {
    const decision = DESIGN_DECISIONS.bioengineeringNonGmoLabelExcluded;
    expect(decision.discrepancy).toMatch(/#1/);
    expect(decision.kind).toBe('correction_narrowed');
    expect(decision.reasoning).toMatch(/legal GMO/i);
    expect(decision.reasoning).toMatch(/point-in-time certification/i);
    expect(decision.reasoning).toMatch(/false negative/i);
    expect(decision.decidedOn).toBe('2026-07-11');
  ...
  test('CORRECTION #2 (revised) — conventional_meat row diverges from live scan.js for game meat, now a GATED GREEN, not a no-op', () => {
    const rule = ruleFor('conventional_meat');
    expect(rule.level2.divergesFromLiveScanJs).toBe(true);
    expect(rule.level2.divergenceReason).toMatch(/Node 6/);
    expect(rule.level2.divergenceReason).toMatch(/gated green/i);
    const gameMeatClearance = rule.clearance.find(c => c.mechanism === 'game-meat-category');
    expect(gameMeatClearance).toBeDefined();
    expect(gameMeatClearance.effect).toBe('gated-green');
  });

  test('GATED GREEN scenario 1 — clean game meat (no other reject flag) is documented as resolving to green', () => {
    const gameMeatClearance = ruleFor('conventional_meat').clearance.find(c => c.mechanism === 'game-meat-category');
    expect(gameMeatClearance.notes).toMatch(/no pre-existing reject-severity flag.*(→|->)\s*green/i);
  });

  test('GATED GREEN scenario 2 — game meat WITH a separate reject-severity flag present is documented as leaving that flag alone (not discarding it)', () => {
    const gameMeatClearance = ruleFor('conventional_meat').clearance.find(c => c.mechanism === 'game-meat-category');
    expect(gameMeatClearance.notes).toMatch(/otherwise left alone/i);
    // The divergenceReason spells out the "never discarded" guarantee explicitly.
    expect(ruleFor('conventional_meat').level2.divergenceReason).toMatch(/never discarding a genuine reject flag/i);
  });

  test('CORRECTION #2 (revised) — both the wild-caught path AND the game-meat path now require the reject-flag gate', () => {
    const rule = ruleFor('conventional_meat');
    expect(rule.rejectFlagGate.required).toBe(GATE_REQUIREMENT.REQUIRED);
    expect(rule.rejectFlagGate.notes).toMatch(/wild-caught-signal path/);
    expect(rule.rejectFlagGate.notes).toMatch(/game-meat-category path/);
    // The note may reference the prior no-op design as history, but must not
    // currently describe game-meat's own path as needing no gate.
    expect(rule.rejectFlagGate.notes).not.toMatch(/game-meat-category path,? because[^.]*no-op[^.]*needs no gate/i);
  });

  test('REVISED (post-Stage-4) — the design decisions log documents WHY game-meat moved from no-op to gated-green', () => {
    const decision = DESIGN_DECISIONS.correctedGameMeatGatedGreenAtL2;
    expect(decision.discrepancy).toBe('#2');
    expect(decision.kind).toBe('correction');
    expect(decision.revisionNote).toMatch(/Stage 4 shadow-mode/i);
    expect(decision.revisionNote).toMatch(/clean.*game-meat product/i);
    expect(decision.revisionNote).toMatch(/L1\/L2/);
    expect(decision.appliedToLiveCode).toBe(false);
  });
...
describe('I. Design decisions log', () => {
  test('all 6 discrepancy decisions are present and correctly numbered (5 from Stage 2, 1 post-Stage-4 revision)', () => {
    const byDiscrepancy = Object.values(DESIGN_DECISIONS).map(d => d.discrepancy).sort();
    expect(byDiscrepancy).toEqual(['#1', '#1 (follow-up, post-Stage-4)', '#2', '#3', '#4', '#6'].sort());
  });

  test('decisions #1, #2, #4 are corrections; #3 is an intentional non-fix; #6 is not carried forward; the post-Stage-4 #1 follow-up is a narrowed correction', () => {
    expect(DESIGN_DECISIONS.correctedBioengineeringOrganicClearance.kind).toBe('correction');
    expect(DESIGN_DECISIONS.bioengineeringNonGmoLabelExcluded.kind).toBe('correction_narrowed');
    expect(DESIGN_DECISIONS.correctedGameMeatGatedGreenAtL2.kind).toBe('correction');
  ...
  test('decision descriptions cross-reference the correct VERDICT_RULES rows', () => {
    ...
    expect(DESIGN_DECISIONS.bioengineeringNonGmoLabelExcluded.tableEncodesInstead).toMatch(/bioengineering/);
    expect(DESIGN_DECISIONS.correctedGameMeatGatedGreenAtL2.tableEncodesInstead).toMatch(/conventional_meat/);
