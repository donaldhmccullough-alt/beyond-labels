...
function analyzeIngredients(ingredientText, productLabels) {
  ...
  // Regex catches: "natural flavor", "natural flavors", "natural and artificial
  // flavor/flavors", and British "flavour" spellings.
  const naturalFlavorMatch = /natural\s+(?:and\s+artificial\s+)?flavou?rs?/.exec(text);
  if (naturalFlavorMatch) {
    softFlags.push({
      category: 'natural_flavors',
      severity: 'caution',
      matchedIngredient: naturalFlavorMatch[0],
      summary:
