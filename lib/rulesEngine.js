...
function analyzeIngredients(ingredientText, productLabels, userLevel = 2) {
  ...
  // Any parsed token not covered by at least one trigger in ANY category is
  // considered unverified — the engine has no knowledge of it either way.
  // These are captured to the database for team review; they don't affect
  // the verdict or flags returned to the user.
  const unverifiedIngredients = parseIngredientTokens(String(ingredientText))
    .filter(token => {
