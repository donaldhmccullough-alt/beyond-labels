...
/**
 * Returns true if the single character immediately before `index` equals
 * `letter`, and that letter itself begins a word (the character before it is
 * not a letter). Used to detect a short trigger embedded inside one specific,
 * confirmed collision word — e.g. bare 'oats' inside "goats" (the 'g' of
 * "goat" immediately precedes the match), bare 'corn' inside "acorn" (the
 * 'a' of "acorn"), bare 'rice' inside "price" (the 'p' of "price").
 *
 * Deliberately narrower than a general "any adjacent letter" boundary guard:
 * a blanket check on bare 'corn' would also incorrectly suppress "popcorn"
 * (the 'p' of "pop" is not the collision letter 'a') — a real, intended
 * corn/gluten match that must keep flagging. Checking for the exact
 * collision letter, itself word-initial, distinguishes "acorn" from
 * "popcorn" and "price" from "brown rice"/bare "rice" without needing a
 * broader rule.
 *
 * @param {string} text   - Full lowercased ingredient string.
 * @param {number} index  - Start position of the matched trigger.
 * @param {string} letter - The single lowercase letter that, when immediately
 *   preceding the match and itself word-initial, indicates the known
 *   false-positive collision this call site is guarding against.
 * @returns {boolean}
 */
function isImmediatelyPrecededByLetter(text, index, letter) {
  const charBefore = index > 0 ? text[index - 1] : null;
  if (charBefore !== letter) return false;
  const charBeforeThat = index > 1 ? text[index - 2] : null;
  return charBeforeThat === null || !/[a-z]/.test(charBeforeThat);
}
...
function analyzeIngredients(ingredientText, productLabels, userLevel = 2) {
  ...
  // bare 'oats'/'oat milk'/'oatmilk' guard: skip when immediately preceded by
  //   word-initial "goat" (prevents false positives in "goats' milk yogurt",
  //   "goat milk", "goatmilk" — none of which contain any actual oats).
  ...
  for (const { trigger, index, end } of findMatches(text, GLYPHOSATE_HEAVY, claimedRanges)) {
    ...
    // embedded inside "goat" (e.g. "goats' milk yogurt", "goat milk",
    // "goatmilk"). Confirmed real false positive: a plain goat-dairy product
    // with zero oat content was producing a reject-severity glyphosate_heavy
    // flag and a false RED verdict. Narrow to the exact "goat" collision
    // (not a blanket adjacent-letter check) so this doesn't touch any other
    // legitimate match.
    if (
      (trigger === 'oats' || trigger === 'oat milk' || trigger === 'oatmilk') &&
      isImmediatelyPrecededByLetter(text, index, 'g')
    ) continue;
  ...
  //
  // Bare 'oats'/'corn'/'rice' collision guards: each skips one specific,
  // confirmed false-positive collision word — 'oats' inside "goats" (no
  // actual oats), 'corn' inside "acorn" (acorn squash has no corn), 'rice'
  // inside "price" (marketing/disclaimer text, not an ingredient). Uses
  // isImmediatelyPrecededByLetter() rather than a blanket adjacent-letter
  // check so legitimate matches like 'corn' inside "popcorn" (real corn
  // concern) and 'rice' inside "brown rice" (a different, longer trigger
  // anyway) are unaffected — see that function's doc comment for why a
  // blanket guard would be unsafe here.
  for (const { trigger, index, end } of findMatches(text, GLUTEN_GRAINS, [])) {
    ...
    if (trigger === 'oats' && isImmediatelyPrecededByLetter(text, index, 'g')) continue;
    if (trigger === 'corn' && isImmediatelyPrecededByLetter(text, index, 'a')) continue;
    if (trigger === 'rice' && isImmediatelyPrecededByLetter(text, index, 'p')) continue;
    flags.push({
