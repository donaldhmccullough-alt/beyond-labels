...
const SYNTHETIC_ADDITIVES = [
  ...
  'sodium acid pyrophosphate',    // leavening acid / SAPP (E450iii)
  ...
  'methylcellulose',         // distinct specific compound, not a substring/synonym of 'modified cellulose'
  ...
  'vanillin',
  // ── Processing methods ────────────────────────────────────────────────────
  // Bare 'mechanically separated' (not '...meat') — structural/processing
  // concern, not a chemical additive. Real labels name the species
  // ("mechanically separated chicken/turkey/pork/beef"), never the generic
  // word "meat", so the original literal 'mechanically separated meat'
  // phrase never matched a real product. A bare, species-agnostic trigger
  // is safe here: at 23 characters this is far too long and distinctive a
  // phrase for any accidental substring collision (unlike short bare
  // triggers such as 'corn'/'oats'/'ada', which needed the dedicated
  // collision guards documented elsewhere in this file) — no plausible
  // English word or real ingredient phrase contains "mechanically
  // separated" as a substring outside this exact processing-method
  // declaration, so no additional word-boundary/adjacency guard is needed.
...
const FORTIFIED_VITAMINS = [
  ...
  'inositol',
  // ── Vitamin C ─────────────────────────────────────────────────────────────
  'sodium ascorbate',       // sodium salt form of vitamin C; distinct from 'ascorbic acid'
                            // (which is tracked separately in CONVENTIONAL_CROPS)
...
/**
 * Bare FORTIFIED_VITAMINS entries confirmed to collide with real, unrelated
 * compound names via plain substring matching — same bug class as the
 * 'ada'/macadamia, 'corn'/popcorn, and 'oats'/goats collisions found
 * elsewhere in this file, just never audited for this list until now.
 *
 * 'calcium phosphate' is a literal substring of 'monocalcium phosphate',
 ...
 * vitamin/mineral fortification additive. Confirmed via a scan_cache audit:
 * 31 real products contain one of these three compounds, 4 of which had
 * already produced a false-positive fortified_vitamins flag (only fires
 * when hasOrganic is also true — see pages/api/scan.js's Phase A block).
 * A separate spot-check of 'calcium carbonate' and 'calcium citrate'
 * against the same full scan_cache dataset found zero collisions for
 * either — left unguarded, no fix needed.
 *
 * @see isAdjacentToLetterUnlessAllowlisted()
 */
const FORTIFIED_VITAMINS_BOUNDARY_GUARDED = new Set([
  'calcium phosphate',
]);

/**
 * Returns true if the ingredient string contains any entry from FORTIFIED_VITAMINS.
 * Case-insensitive substring match. Entries in FORTIFIED_VITAMINS_BOUNDARY_GUARDED
 * are additionally checked via isAdjacentToLetterUnlessAllowlisted() so a bare
 * trigger embedded inside a larger, unrelated compound name (e.g. 'calcium
 * phosphate' inside 'monocalcium phosphate') is not treated as a match.
 * @param {string} ingredients
 * @returns {boolean}
 */
function containsFortifiedVitamins(ingredients) {
  const lower = ingredients.toLowerCase();
  return FORTIFIED_VITAMINS.some(trigger => {
    if (!FORTIFIED_VITAMINS_BOUNDARY_GUARDED.has(trigger)) {
      return lower.includes(trigger);
    }
    let idx = lower.indexOf(trigger);
    while (idx !== -1) {
      if (!isAdjacentToLetterUnlessAllowlisted(lower, idx, idx + trigger.length)) {
        return true;
      }
...
const ALWAYS_IGNORE_INGREDIENTS = [
  ...
  'calcium carbonate',
  'magnesium oxide',
  'potassium chloride',   // common salt substitute, not a synthetic additive of concern
  // Yeast forms
  'nutritional yeast',
  'active dry yeast',
  'instant yeast',
  ...
  'rennet',
  'pectin',
  // Plain recognized whole foods (not a concern of any kind — display-only suppression)
  'coffee',
];
...
const ALL_TRIGGERS = [
  ...
  ...NATURAL_FLAVORS,
  ...SYNTHETIC_ADDITIVES,
  ...MEAT_INGREDIENT_TERMS,     // prevents named meat cuts (e.g. "chicken breast",
                                // "angus beef") from appearing in unverified —
                                // these are already corroborated by containsMeatIngredient()
  ...MEAT_DERIVED_INGREDIENTS,  // same reasoning — gelatin forms already corroborated
                                // by containsMeatDerived()
  ...MILK_DERIVED_INGREDIENTS,  // same reasoning — dairy forms already corroborated
                                // by containsMilkDerived()
  ...GLUTEN_GRAINS,
  ...FORTIFIED_VITAMINS,   // prevents vitamin/mineral names from appearing in unverified
  ...NATURAL_COLORANTS,    // prevents plant-derived colorant names from appearing in unverified —
                           // already corroborated by containsNaturalColorants() in the L2 organic path
];
 ...
 * anchor following the same period-terminated style. (3) "Manufactured/
 * Made/Processed in a facility that (also) processes X." — a distinct
 * facility cross-contact disclosure shape from (2): no "this product is"
 * prefix, and the verb ("manufactured"/"made"/"processed") sits directly
 * before "in a facility". Confirmed as a real false-flag source, not just a
 * cosmetic one: "Manufactured in a facility that also processes egg, soy,
 * sesame, and tree nuts." (barcode 810104462914) produced a false
 * conventional_eggs reject flag with no real egg ingredient anywhere else
 * in the list. Also confirmed on real scan_cache data: "Made in a facility
 * that also processes other tree nuts, egg, wheat and sesame." (barcode
 * 749826000497) and, without "also", "MADE IN A FACILITY THAT ALSO
 * PROCESSES SOY, EGGS, MILK, COCONUT." (barcode 819898010080) — the "also"
 * is therefore optional in the pattern. A fourth real occurrence
 * ("PROCESSED IN A FACILITY THAT PROCESSES TREE NUTS AND EGGS...", barcode
 * 850009682130) has no terminating period anywhere in the rest of the
 * string at all (a heavily OCR-corrupted nutrition-panel-in-ingredients_text
 * case — see CLAUDE.md's known-issues note on barcode 887422000210 for the
 * same underlying source-data problem) — confirmed this pattern's
 * period-terminated `[^.]*\.` simply fails to match that occurrence rather
 * than running away to some distant, unrelated period, so it's left
 * unstripped exactly like an unterminated "Manufactured by:" is. No
 * end-of-string fallback was added for the same reason "Manufactured by:"
 * doesn't have one — real ingredient text could follow with no separator.
 *
...
function stripAllergenAdvisory(str) {
  return str
    ...
    .replace(/\b(?:manufactured|made|processed)\s+in\s+a\s+facility\s+that\s+(?:also\s+)?process(?:es)?[^.]*\./gi, '')
    .replace(/\.\s*(?:usda\s+)?(?:certified\s+)?organic\.?\s*$/gi, '')
    // Bare "contains" pattern 1 of 2 — qualifier phrase only, MUST run
    // before pattern 2. Covers two real-world quantity phrasings — "contains
    // less than X% of" and "contains X% or less of" (the latter equally
    // common and, before this fix, unrecognized here entirely, causing it to
    // fall through to pattern 2's greedy fallback below and silently delete
    // every real ingredient after "contains" up to the string's final
    // period). The optional colon can sit directly against "of" with no
    // space ("...of: salt") or after "the following" ("...of the
    // following: salt") — `\s*(?:the\s+following)?:?\s*` handles both
    // orderings plus the comma-terminated and bare period-terminated forms
    // without ever leaving a stray "the following" token behind.
    .replace(/contains\s+(?:less\s+than\s+[\d.]+%|[\d.]+%\s+or\s+less)\s+of\s*(?:the\s+following)?:?\s*/gi, '')
 ...
 *
 * 'sweetcorn' (bare 'corn' inside "sweetcorn") was found during the
 * systematic bare-trigger audit's verification pass, not the audit itself
 * — the audit only searches for FALSE POSITIVES (words that incorrectly
 * flag), so it can't surface this class of bug: a legitimate compound
 * ("sweetcorn" genuinely is corn) that the SUFFIX-collision guard has been
 * silently over-suppressing since the 'corn' guard was first added two
 * sessions ago. GLUTEN_GRAINS has no dedicated 'sweetcorn' trigger of its
 * own (only the two-word 'sweet corn' would be a separate string), so like
 * 'groats', it relied entirely on the bare 'corn' match — which the
 * before-only default was blocking, since "sweetcorn" has a letter
 * immediately before ('t') and wasn't allowlisted. Confirmed via direct
 * testing that this was a real, live false negative, not a hypothetical.
 *
 * 'cowpeas', 'broadbeans', and 'horsebeans' (bare 'peas'/'beans' inside
 * these one-word compounds) were found by a systematic false-negative
 * sweep — the mirror image of the audit above, checking every guarded
 * trigger's dictionary hits for a legitimate compound now silently losing
 * its flag, rather than an unrelated word incorrectly gaining one.
 * "Cowpeas" (black-eyed peas) and "broad beans"/"horse beans" (fava beans
 * and a horse-feed bean variety) are all real edible legumes — GLYPHOSATE_HEAVY
 * has no dedicated one-word trigger for any of them, so each relied
 * entirely on the bare 'peas'/'beans' match, which the SUFFIX-collision
 * guard was blocking (a letter immediately precedes the match in all
 * three: "cow", "broad", "horse"). The two-word spaced forms ("black eyed
 * peas", "broad beans", "horse beans") are unaffected either way, since a
 * space before the trigger is not a letter and was never blocked.
 */
...
/**
 * Returns true if a bare trigger match is immediately adjacent to a letter
 ...
 *
 * By default only the PRECEDING character is checked — this matches every
 * confirmed collision found so far except one: 'peas' inside "peasant" is a
 * PREFIX collision (letters follow the trigger, none precede it — "peasant"
 * starts with "peas"), the mirror image of every other case (oats/corn/rice/
 * spelt, all SUFFIX collisions — the collision word ends with the trigger).
 * Pass `checkAfter: true` to also block on a following letter, for triggers
 * with a confirmed prefix-collision. This is opt-in, not the default, because
 * checking the following character too would also incorrectly block real
 * no-space label variants already confirmed to rely on a bare trigger's
 * suffix-only match — e.g. "cornstarch" (bare 'corn' followed by 's') and
 * "ricecake" (bare 'rice' followed by 'c') are both genuine corn/rice
 * ingredients, not collisions, and must keep matching via the "before-only"
 * default.
 *
 * @param {string}  text        - Full lowercased ingredient string.
 * @param {number}  index       - Start position of the matched trigger.
 * @param {number}  end         - End position of the matched trigger.
 * @param {boolean} [checkAfter=false] - Also block when the character
 *   immediately following the match is a letter (for confirmed prefix
 *   collisions like 'peas'/"peasant"). Leave false for suffix collisions.
 * @returns {boolean}
 */
function isAdjacentToLetterUnlessAllowlisted(text, index, end, checkAfter = false) {
  const charBefore = index > 0 ? text[index - 1] : null;
  const hasLetterBefore = Boolean(charBefore) && /[a-z]/.test(charBefore);

  const charAfter = checkAfter && end < text.length ? text[end] : null;
  const hasLetterAfter = Boolean(charAfter) && /[a-z]/.test(charAfter);

  if (!hasLetterBefore && !hasLetterAfter) return false;
...
/**
 * Confirmed-BAD compound words for bare 'corn' — words that look like they
 * might be about corn but genuinely are not (spatial "corner", eye anatomy
 * "cornea", the musical instrument "cornet", architectural "cornice", the
 * pickle "cornichon", "corned" as in corned beef — historically from grains
 * of curing salt, unrelated to the maize crop — the common word "scorn", the
 * spice "peppercorn", the wildflower "cornflower", the hairstyle "cornrow").
 *
 * This is the inverse of TRIGGER_ADJACENCY_ALLOWLIST, used only for bare
 * 'corn': unlike every other guarded trigger, legitimate PREFIX-shape corn
 * compounds (cornbread, cornmeal, cornstarch, cornflakes, corncob,
 * cornstalk, cornhusk, cornfield, sweetcorn, popcorn...) vastly outnumber
 * genuine collisions, so blocking-by-default-with-an-allowlist would need
 * dozens of allowlist entries. A small denylist of the confirmed-bad words
 * is far less code and far less risk of accidentally suppressing a real
 * corn concern.
 *
 * Not exhaustive — omits extremely obscure/archaic dictionary entries
 * (heraldry, entomology, geology terms like "cornigerous", "cornuted",
 * "lamellicorn", "cornbrash") that are vanishingly unlikely to ever appear
 * in real product text. Extend if a new real-world false positive surfaces.
 */
const CORN_COLLISION_DENYLIST = new Set([
  'corner', 'corners', 'cornered', 'cornering',
  'cornerback', 'cornerbacks', 'cornerman', 'cornermen',
  'cornerstone', 'cornerstones', 'cornerways', 'cornerwise',
  'cornea', 'corneas', 'corneal',
  'cornet', 'cornets', 'cornetist', 'cornetists', 'cornetcy', 'cornetcies',
  'cornett', 'cornetti', 'cornettini', 'cornettino', 'cornetto', 'cornetts',
  'cornettist', 'cornettists',
  'cornice', 'cornices', 'corniced',
  'corniche', 'corniches',
  'cornichon', 'cornichons',
  'corned',
  'scorn', 'scorned', 'scorner', 'scorners',
  'scornful', 'scornfully', 'scornfulness', 'scornfulnesses',
  'scorning', 'scornings', 'scorns',
  'peppercorn', 'peppercorns', 'peppercorny',
  'cornflower', 'cornflowers',
  'cornrow', 'cornrowed', 'cornrowing', 'cornrows',
]);

/**
 * Returns true if a PREFIX-shape bare-trigger match (no letter immediately
 * before, one or more letters after) forms a word on
 * CORN_COLLISION_DENYLIST. Only checks the PREFIX direction — bare 'corn'
 * SUFFIX collisions ("acorn", "unicorn") are already handled by the
 * before-only default of isAdjacentToLetterUnlessAllowlisted().
 *
 * @param {string} text  - Full lowercased ingredient string.
 * @param {number} index - Start position of the matched trigger.
 * @returns {boolean}
 */
function isDenylistedCornCollision(text, index) {
  const charBefore = index > 0 ? text[index - 1] : null;
  if (charBefore && /[a-z]/.test(charBefore)) return false; // not a prefix match

  let wordEnd = index;
  while (wordEnd < text.length && /[a-z]/.test(text[wordEnd])) wordEnd++;
  const word = text.slice(index, wordEnd);

  return CORN_COLLISION_DENYLIST.has(word);
}
...
function analyzeIngredients(ingredientText, productLabels, userLevel = 2) {
  ...
  for (const { trigger } of matchAndClaim(TRANS_FATS)) {
    // Bare-"hydrogenated" summary wording approved as final — see CLAUDE.md
    // changelog entry for this session's trans_fats explanation fix.
    const isBareHydrogenated = trigger === 'hydrogenated';
    flags.push({
      category: 'trans_fats',
      severity: 'reject',
      matchedIngredient: trigger,
      summary: isBareHydrogenated
        ...
        : `Contains "${trigger}" — the primary source of artificial trans fats, though the ` +
  ...
  for (const { trigger, index, end } of findMatches(text, CONVENTIONAL_CROPS, claimedRanges)) {
    ...
    // Bare 'miso' collision guard — MIDDLE-shape collision ("semisoft", a
    // real cheese classification term, has letters on both sides of the
    // match), so the before-only default of isAdjacentToLetterUnlessAllowlisted
    // is sufficient (no checkAfter needed). CONVENTIONAL_CROPS's own 'miso'
    // entry has the same substring-collision shape as GLUTEN_GRAINS's
    // separate 'miso' trigger — both fixed together this session.
    if (trigger === 'miso' && isAdjacentToLetterUnlessAllowlisted(text, index, end)) continue;
  ...
  // Confirmed real false positive: this regex's negative lookahead only
  // excluded "corn" followed by whitespace+letter ("corn starch" as two
  // words) — not "corn" followed directly by more letters. "corner",
  // "cornea", "cornet", "cornice", "cornichon", "corned" (as in corned
  // beef), "scorn", "peppercorn" and more were all matching and producing a
  // false reject-severity conventional_crops flag and a false RED verdict.
  // Guarded via CORN_COLLISION_DENYLIST (isDenylistedCornCollision) rather
  // than tightening the lookahead to reject any following letter — that
  // would also break legitimate no-space compounds this regex is relied on
  // to catch (e.g. "cornstarch", "cornmeal" as one word).
  const STANDALONE_CORN_RE = /(?:^|,)\s*(organic\s+)?corn(?!\s+[a-z])/;
  const cornMatch = STANDALONE_CORN_RE.exec(text);
  if (cornMatch && !isDenylistedCornCollision(text, cornMatch.index + cornMatch[0].indexOf('corn'))) {
  ...
  //
  // SUFFIX-shape collisions (collision word ends with the trigger) use the
  // before-only default of isAdjacentToLetterUnlessAllowlisted(): 'oats'/
  // 'oat milk'/'oatmilk' inside "goats' milk yogurt"/"goat milk"/"goatmilk"/
  // "coats"; 'spelt' inside "misspelt"; 'rye' inside "fryer"/"dryer"; 'flax'
  // inside "toadflax"; 'beans' inside "jellybeans".
  //
  // PREFIX-shape collisions (collision word starts with the trigger) need
  // checkAfter: true: 'peas' inside "peasant" (e.g. "peasant bread");
  // 'farro' inside "farrow"/"farrowing" (pig-birthing terminology, unrelated
  // to the farro grain). 'malt' needs checkAfter too — this REPLACES the
  // previous narrower inline guard, which only checked the character AFTER
  // 'malt' (protecting "maltodextrin"/"maltose"). That one-sided check
  // missed "smalt" (a blue pigment/cobalt glass): a letter BEFORE 'malt'
  // ('s') but nothing after, the mirror-image gap. isAdjacentToLetterUnlessAllowlisted
  // with checkAfter: true covers both directions at once, closing that gap
  // while still protecting maltodextrin/maltose exactly as before.
  ...
  for (const { trigger, index, end } of findMatches(text, GLYPHOSATE_HEAVY, claimedRanges)) {
    ...
    if (
      (trigger === 'oats' || trigger === 'oat milk' || trigger === 'oatmilk' ||
        trigger === 'spelt' || trigger === 'rye' || trigger === 'flax' || trigger === 'beans') &&
      isAdjacentToLetterUnlessAllowlisted(text, index, end)
    ) continue;

    if (
      (trigger === 'peas' || trigger === 'farro' || trigger === 'malt') &&
      isAdjacentToLetterUnlessAllowlisted(text, index, end, true)
    ) continue;
    ...
    flags.push({
      ...
      summary:
        ...
        `pre-harvest desiccant. A glyphosate-free or USDA Organic label can point toward ` +
        `lower exposure, though it's not a guarantee.`,
    });
  }
  ...
  //
  // 'bha' and 'olean' are both PREFIX-shape collisions (letters follow, none
  // precede) — 'bha' inside "bhaji"/"bhajia"/"sambhar" (real South Asian
  // culinary terms) and 'olean' inside "oleander" (carried over from an
  // earlier session's audit, confirmed again here). Use
  // isAdjacentToLetterUnlessAllowlisted() with checkAfter: true, the same
  // mechanism as 'peas'/'farro'/'malt' elsewhere in this file.
  for (const { trigger, index, end } of matchAndClaim(SYNTHETIC_ADDITIVES)) {
  ...
  //
  // Bare-trigger collision guards — SUFFIX-shape triggers (collision word
  // ends with the trigger) use the before-only default of
  // isAdjacentToLetterUnlessAllowlisted(): 'oats' inside "goats"/"coats",
  // 'corn' inside "acorn"/"unicorn", 'rice' inside "price"/"licorice",
  // 'spelt' inside "misspelt", 'rye' inside "fryer"/"dryer", 'miso' inside
  // "semisoft" (a real cheese classification term — GLUTEN_GRAINS has its
  // own separate 'miso' entry alongside CONVENTIONAL_CROPS's, both fixed
  // together). Legitimate compounds ('corn' inside "popcorn"/"sweetcorn",
  // 'oats' inside "groats") are carved out via TRIGGER_ADJACENCY_ALLOWLIST;
  // see that function's doc comment.
  //
  // PREFIX-shape triggers (collision word starts with the trigger) need
  // checkAfter: true — 'hing' inside "hinge"/"hinges"/"hinged" (a hardware
  // term, alternative name for asafoetida has no known legitimate
  // letter-adjacent compound, so no allowlist entry needed), 'malt' also
  // needs checkAfter since it's a PREFIX risk too ("smalt", a pigment, has
  // a letter before but not after — see the GLYPHOSATE_HEAVY loop's malt
  // comment for the full history), 'farro' inside "farrow"/"farrowing"
  // (pig-birthing terminology, unrelated to the farro grain).
  //
  // Bare 'corn' ALSO gets isDenylistedCornCollision() on top of the
  // suffix-only default — PREFIX-shape corn collisions ("corner", "cornea",
  // "cornet", "cornice", "cornichon", "corned", "scorn", "peppercorn") are
  // NOT handled via checkAfter/allowlist here, because legitimate PREFIX
  // corn compounds (cornbread, cornmeal, cornstarch, cornflakes, corncob...)
  // vastly outnumber the collisions; see CORN_COLLISION_DENYLIST's doc
  // comment for the full rationale. The same collision also affects the
  // standalone-corn regex in CONVENTIONAL_CROPS below, fixed the same way.
  for (const { trigger, index, end } of findMatches(text, GLUTEN_GRAINS, [])) {
    ...
    if (
      (trigger === 'oats' || trigger === 'rice' || trigger === 'spelt' ||
        trigger === 'rye' || trigger === 'miso') &&
      isAdjacentToLetterUnlessAllowlisted(text, index, end)
    ) continue;
    if (
      (trigger === 'hing' || trigger === 'malt' || trigger === 'farro') &&
      isAdjacentToLetterUnlessAllowlisted(text, index, end, true)
    ) continue;
    if (
      trigger === 'corn' &&
      (isAdjacentToLetterUnlessAllowlisted(text, index, end) || isDenylistedCornCollision(text, index))
    ) continue;
    flags.push({
