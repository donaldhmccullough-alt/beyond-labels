...
function analyzeIngredients(ingredientText, productLabels, userLevel = 2) {
  ...
  // bare 'oats'/'oat milk'/'oatmilk'/'spelt'/'peas' guard: skip when embedded
  //   inside a larger word (see isAdjacentToLetterUnlessAllowlisted) — covers
  //   "goats' milk yogurt", "goat milk", "goatmilk", "coats", "misspelt",
  //   "peasant bread", etc.
  ...
  for (const { trigger, index, end } of findMatches(text, GLYPHOSATE_HEAVY, claimedRanges)) {
    ...
    // Word-boundary guard for 'oats'/'oat milk'/'oatmilk'/'spelt' — SUFFIX
    // collisions only (the collision word ends with the trigger), so the
    // before-only default is correct: "goats' milk yogurt", "goat milk",
    // "goatmilk", "coats", "misspelt". Confirmed real false positives: a
    // plain goat-dairy product with zero oat content, and "misspelt" with
    // zero spelt content, were producing reject-severity glyphosate_heavy
    // flags and false RED verdicts. No trigger here needs an allowlist entry
    // (unlike bare 'corn' and "popcorn" below) — none is a legitimate suffix
    // of another real word the way corn/popcorn or oats/groats are.
    if (
      (trigger === 'oats' || trigger === 'oat milk' || trigger === 'oatmilk' || trigger === 'spelt') &&
      isAdjacentToLetterUnlessAllowlisted(text, index, end)
    ) continue;

    // Word-boundary guard for 'peas' — a PREFIX collision ("peasant" starts
    // with "peas", the trigger has no letter before it and letters after),
    // the mirror image of every other guarded trigger in this file. Requires
    // checkAfter: true. Confirmed real false positive: "peasant bread" (a
    // real bread product/style name) with zero pea content was producing a
    // reject-severity glyphosate_heavy flag and a false RED verdict.
    if (trigger === 'peas' && isAdjacentToLetterUnlessAllowlisted(text, index, end, true)) continue;
  ...
  //
  // Bare 'oats'/'corn'/'rice'/'spelt'/'hing' collision guard: skips any of
  // these five bare triggers when embedded inside a larger word — e.g.
  // 'oats' inside "goats" or "coats", 'corn' inside "acorn" or "unicorn",
  // 'rice' inside "price" or "licorice", 'spelt' inside "misspelt", 'hing'
  // inside "something"/"anything"/"everything"/"nothing" — via
  // isAdjacentToLetterUnlessAllowlisted(). All five are SUFFIX collisions
  // (the collision word ends with the trigger), so the before-only default
  // is correct here — unlike 'peas'/"peasant" in the GLYPHOSATE_HEAVY loop
  // above, a PREFIX collision that needs checkAfter: true. Legitimate
  // compounds (bare 'corn' inside "popcorn", bare 'oats' inside "groats")
  // are carved out via TRIGGER_ADJACENCY_ALLOWLIST rather than narrowing the
  // guard itself; see that function's doc comment for the full rationale.
  // 'hing' (alternative name for asafoetida) has no known legitimate
  // letter-adjacent compound — confirmed real matches ("hing", "hing
  // powder", "asafoetida", parenthetical "(hing)") are all either standalone
  // or space/punctuation-separated, so no allowlist entry was needed for it.
