 ...
 * scan.js does today:
 *
 *   - bioengineering: gains organic-label and organic-prefix clearance
 *     (today's rulesEngine.js has none — see
 *     DESIGN_DECISIONS.correctedBioengineeringOrganicClearance, discrepancy
 *     #1). Non-GMO Project Verified label clearance was in the original
 *     Stage 3 draft too, then deliberately removed after Stage 4 shadow-mode
 *     review — see DESIGN_DECISIONS.bioengineeringNonGmoLabelExcluded.
 *   - conventional_meat: game-meat detection gets a GATED GREEN at L2,
 *     mirroring the wild-caught path exactly — game meat + no pre-existing
 *     reject flag → green; a reject flag present is left alone, never
 *     discarded (see DESIGN_DECISIONS.correctedGameMeatGatedGreenAtL2,
 *     discrepancy #2). The original Stage 3 draft modeled this as a full
 *     no-op removed from the priority chain; Stage 4 shadow-mode comparison
 *     surfaced an undecided side effect (a clean game-meat product losing
 *     its automatic green), so this was narrowed to gated-green instead.
...
const VERDICT_RULES = [
  ...
  },
  {
    ...
    level2: {
      ...
      divergenceReason:
        "CORRECTED per Stage 2 discrepancy #1. Today's lib/rulesEngine.js BIOENGINEERING_TERMS loop has NO organic/prefix clearance at all — it is the one " +
        'reject-capable category with no engine-level clearance path, which is the direct root cause of the live Node 4 bug (a usda-organic-labeled product ' +
        'with a bioengineering disclosure gets a full GREEN, silently discarding the reject flag) documented in the Stage 2 report. This row gives ' +
        "bioengineering organic-label and organic-prefix clearance — conventional_eggs/glyphosate_heavy's narrower pattern, not conventional_crops's fuller " +
        "one (see the non-gmo exclusion below, added after Stage 4 shadow-mode review — see DESIGN_DECISIONS.bioengineeringNonGmoLabelExcluded).",
    },
    clearance: [
      ...
      { mechanism: 'non-gmo-project-verified-label', effect: 'INTENTIONALLY-NOT-CLEARED', notes:
        'REVISED after Stage 4 shadow-mode review (2026-07-11) — originally full-clearance in the first Stage 3 draft, deliberately narrowed to exclude this ' +
        'mechanism. See DESIGN_DECISIONS.bioengineeringNonGmoLabelExcluded for the full reasoning: unlike USDA organic certification (a legal GMO prohibition), ' +
        'Non-GMO Project Verified is a private, point-in-time certification that can go stale relative to a product\'s current formulation, and bioengineering ' +
        'is a reject-severity flag — the app\'s strongest signal — so a false-negative here is worse than an over-cautious verdict.' },
      { mechanism: 'organic-ingredient-prefix', effect: 'full-clearance' },
    ],
    rejectFlagGate: {
      required: GATE_REQUIREMENT.NOT_REQUIRED,
      notes: 'For organic-labeled/organic-prefixed products, bioengineering is cleared at the engine layer (this row\'s clearance), so there is no stray reject ' +
        'flag for the organic path to silently discard. For non-gmo-labeled-only products (no longer cleared — see clearance above), the flag survives, but ' +
        "Node 7 (non-gmo) already has its own independent reject-flag gate and Node 6 (game meat) gets one under this table's gated-green correction (#2) — " +
        'so no additional gate is needed on this row itself.',
    },
    scope: SCOPE.GLOBAL,
    notes: 'See level2.divergenceReason above for the original correction rationale (Stage 2 discrepancy #1) and DESIGN_DECISIONS.bioengineeringNonGmoLabelExcluded for the Stage 4 follow-up narrowing.',
  },
  {
  ...
  },
  {
    ...
    level2: {
      ...
      divergenceReason:
        ...
        "gives game-meat detection a GATED GREEN, mirroring Node 5's own wild-caught pattern exactly: game-meat category present AND no pre-existing reject-" +
        "severity flag → green; if a reject flag IS present, it is left alone (not discarded) and the rest of the chain evaluates normally (see " +
        "DESIGN_DECISIONS.correctedGameMeatGatedGreenAtL2). REVISED after Stage 4 shadow-mode review (2026-07-11) — an earlier draft of this row modeled this " +
        "as a full no-op removed from the priority chain entirely; that turned out to have an undecided side effect (a clean game-meat product would fall " +
        "through to the default-yellow node instead of getting an automatic green, a new L1/L2 asymmetry nobody had actually decided on). Gated green " +
        'preserves the automatic clean-verdict for game meat while still never discarding a genuine reject flag underneath it.',
    },
    clearance: [
      ...
      { mechanism: 'wild-caught-signal', effect: 'gated-green', notes: 'detectWildCaught() true for a seafood product, gated on no pre-existing reject-severity flag — no injection, verdict actively set to green when the gate passes.' },
      { mechanism: 'game-meat-category', effect: 'gated-green', notes: 'REVISED (discrepancy #2, narrowed after Stage 4 review) — same gated-green shape as the wild-caught path, not a full no-op: game-meat category present AND no pre-existing reject-severity flag → green; otherwise left alone for the rest of the chain to evaluate.' },
    ],
    rejectFlagGate: {
      required: GATE_REQUIREMENT.REQUIRED,
      notes:
        'Required for BOTH the wild-caught-signal path (already correctly implemented in live L2 Node 5 — the gate that was missing before PROMPT_VERSION 29) ' +
        'AND the game-meat-category path (REVISED after Stage 4 review — previously modeled as a no-op needing no gate; now a gated-green mechanism that ' +
        'actively sets verdict, so it needs the same reject-flag gate as wild-caught, for the same reason).',
    },
    scope: SCOPE.GLOBAL,
    priorityRelativeTo: [
      { category: 'conventional_eggs', relation: 'yields-priority-to', appliesAtLevels: [1, 2] },
    ],
    notes: 'CORRECTED: game-meat detection is a gated green at L2 now (discrepancy #2, revised after Stage 4 review — see DESIGN_DECISIONS.correctedGameMeatGatedGreenAtL2), and this injection yields priority to conventional_eggs at both levels whenever both would otherwise apply (discrepancy #4).',
  },
  {
...
const DESIGN_DECISIONS = {
  correctedBioengineeringOrganicClearance: {
    discrepancy: '#1',
    kind: 'correction',
    liveScanJsBug: "L2 Node 4 (organic path) has no reject-flag gate and lib/rulesEngine.js's BIOENGINEERING_TERMS loop has no organic/prefix clearance at all — a usda-organic-labeled product with a bioengineered disclosure gets a full GREEN, silently discarding the reject flag.",
    tableEncodesInstead: "bioengineering row gains organic-label and organic-prefix clearance, matching conventional_eggs/glyphosate_heavy's pattern — see the row's level2.divergenceReason. (Non-GMO Project Verified label clearance was also granted in the original Stage 3 draft, then removed — see bioengineeringNonGmoLabelExcluded below.)",
    appliedToLiveCode: false,
  },
  bioengineeringNonGmoLabelExcluded: {
    discrepancy: '#1 (follow-up, post-Stage-4)',
    kind: 'correction_narrowed',
    decidedOn: '2026-07-11',
    reasoning:
      'Explicit product decision made after reviewing Stage 4 shadow-mode findings, not an oversight. USDA organic certification is a legal GMO ' +
      'prohibition — a product carrying both a usda-organic label and a bioengineering disclosure is almost certainly a data/mislabeling artifact, so ' +
      'full clearance there remains safe and is unchanged. Non-GMO Project Verified is different in kind: a private, point-in-time certification based ' +
      "on ingredient testing, which can go stale relative to a product's current formulation — particularly now that the federal Bioengineered Food " +
      "Disclosure Standard requires many products to carry an explicit, currently-accurate disclosure phrase independent of any private cert's testing " +
      'date. bioengineering is a reject-severity flag, the strongest signal this app produces — the cost of a false negative (fully clearing a real, ' +
      "current disclosure because of a possibly-stale private label) is worse than the cost of an over-cautious verdict on a product whose label happens " +
      'to still be accurate.',
    tableEncodesInstead: "bioengineering row's clearance list marks non-gmo-project-verified-label as INTENTIONALLY-NOT-CLEARED (the original Stage 3 draft treated it as full-clearance, matching conventional_crops — this entry narrows that to conventional_eggs/glyphosate_heavy's organic-only pattern instead).",
    appliedToLiveCode: false,
  },
  correctedGameMeatGatedGreenAtL2: {
    discrepancy: '#2',
    kind: 'correction',
    ...
    tableEncodesInstead: "conventional_meat row's game-meat-category clearance path is a GATED GREEN, mirroring the wild-caught path exactly: game-meat category present AND no pre-existing reject-severity flag → green; otherwise left alone for the rest of the chain to evaluate — see the row's level2.divergenceReason.",
    revisionNote:
      'REVISED after Stage 4 shadow-mode review (2026-07-11) — the original Stage 3 draft modeled this as a full no-op removed from the priority chain ' +
      'entirely. Shadow-mode comparison against snapshot-baseline.json surfaced an undecided side effect: removing the branch entirely meant a CLEAN ' +
      "game-meat product (zero other flags) fell through to the default-yellow node instead of getting an automatic green, creating a new L1/L2 " +
      "asymmetry (L1's equivalent no-op leaves a clean game-meat product green via the engine's own default) that nobody had actually decided on. Gated " +
      'green fixes this: it preserves the automatic clean-verdict for game meat while still never discarding a genuine reject flag underneath it — the ' +
      'part of the original correction that actually mattered.',
    appliedToLiveCode: false,
  },
