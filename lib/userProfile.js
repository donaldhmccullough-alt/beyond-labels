...
// ── Scan-specific localStorage wipe ───────────────────────────────────────────
// Call on sign-out and after Supabase migration.
// Deliberately does NOT clear bl_profile (stage/flags), bl_nudges (preferences).
const SCAN_KEYS = ['bl_scans', 'bl_scan_history', 'bl_total_scans', 'bl_migrated'];

export function clearScanLocalStorage() {
  if (typeof window === 'undefined') return;
  SCAN_KEYS.forEach(k => localStorage.removeItem(k));
}
