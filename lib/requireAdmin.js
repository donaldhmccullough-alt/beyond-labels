...
export async function requireAdmin(req) {
  ...
  try {
    ...
    // No email/user available yet at this point (the call itself failed) —
    // nothing personal to tag, just visibility that Supabase Auth verification
    // itself is erroring rather than cleanly rejecting the token.
    Sentry.captureException(err, {
      tags: { route: 'requireAdmin', op: 'verify_token' },
    });
    return null;
  }
