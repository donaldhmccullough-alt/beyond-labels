  ...
  if (userLevel === 1) {
    ...
    if (isMeat && verdict !== 'unverified') {
      if (l1IsSeafood && detectWildCaught(productName, labelsDetected, ingredientText)) {
        ...
        flags = [{ category: 'conventional_meat', severity: 'caution', matchedIngredient: '', summary, meatScenario: l1IsSeafood ? 'seafood' : 'land_animal' }, ...flags];
  ...
  if (userLevel === 2 && verdict !== 'unverified' && verdict !== 'inconclusive') {
    ...
    if (hasInstantRedFlag) {
      ...
      if (
        ...
        verdict = 'red';
        clearedBy = null;
        flags = [{ category: 'conventional_meat', severity: 'reject', matchedIngredient: '', summary: 'Farmed or unlabeled seafood — wild-caught certification not found', meatScenario: 'seafood' }, ...flags];
      ...
      // before Node 8b).
        ...
        verdict = 'red';
        clearedBy = null;
        flags = [{ category: 'conventional_meat', severity: 'reject', matchedIngredient: '', summary: 'Conventional meat product without USDA Organic certification', meatScenario: 'land_animal' }, ...flags];

      } else if (maskedText && containsMeatDerived(maskedText)) {
        verdict = 'red';
        clearedBy = null;
        flags = [{ category: 'conventional_meat', severity: 'reject', matchedIngredient: '', summary: 'Contains animal-derived gelatin without organic certification.', meatScenario: 'gelatin' }, ...flags];
