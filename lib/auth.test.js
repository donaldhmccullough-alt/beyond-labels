...
const { signOut, requestPasswordReset, updatePassword, resendConfirmationEmail } = require('./auth');
...
// Added alongside a "check your email" / resend confirmation screen in
// AuthModal.jsx, prompted by the same category of gap the forgot-password
// feature closed at login: a user who signs up and never gets the
// confirmation email (spam filter, typo, slow delivery) previously had no
// path forward at all. Confirmed via a live query against this project's
// Supabase auth settings endpoint (mailer_autoconfirm: false) that email
// confirmation is actually required here, so signUp() genuinely does leave
// the user unconfirmed with no session — this isn't a moot feature.
describe('lib/auth resendConfirmationEmail()', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    global.window = { location: { origin: 'https://beyondlabels.example' } };
  });

  test('calls supabase.auth.resend with type: signup, the email, and an emailRedirectTo matching signUp()\'s own', async () => {
    ...
    expect(supabase.auth.resend).toHaveBeenCalledWith({
      ...
      options: { emailRedirectTo: 'https://beyondlabels.example/auth/callback' },
    });
  });
  ...
  test('propagates a returned error (e.g. server-side rate limiting) without throwing', async () => {
    supabase.auth.resend.mockResolvedValue({
      data: null,
      error: Object.assign(new Error('For security purposes, you can only request this after 57 seconds.'), { status: 429 }),
    });

    const result = await resendConfirmationEmail('user@example.com');

    expect(result.error).toBeTruthy();
    expect(result.error.message).toBe('For security purposes, you can only request this after 57 seconds.');
