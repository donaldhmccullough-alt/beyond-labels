...
For trans_fats — when the flagged trigger is specifically "partially hydrogenated," "margarine," or "shortening" (a confirmed trans fat source, not bare "hydrogenated," which is handled separately): Sina explains that this product lists partially hydrogenated oil, the primary source of artificial trans fats. The evidence became so strong that the FDA revoked their GRAS status in 2015 and banned partially hydrogenated oils outright by 2018 — one of the few times regulators fully reversed course on a widely used ingredient. Frame this as regulators acting decisively on strong evidence, not as another example of lax GRAS oversight — seeing it in a product today likely means an older formulation, an imported product, or a labeling gap, worth a closer look either way. 2-3 sentences.
...
export function buildUserMessage(verdict, flags, productName, ingredients, userLevel = 2, clearedBy = null, unverifiedReason = null) {
  ...
  const categoryLines = Object.entries(byCategory).map(([cat, catFlags]) => {
    ...
    if (cat === 'conventional_meat') {
      const scenario = catFlags[0]?.meatScenario;
      if (scenario === 'seafood') {
        line += '\n    [Meat note: this flag was triggered by farmed or unlabeled seafood, not land-animal meat — do not use feedlot/hormone language. Focus on wild-caught vs. farmed sourcing instead.]';
      } else if (scenario === 'gelatin') {
        line += '\n    [Meat note: this flag was triggered by animal-derived gelatin in a non-meat product (e.g. candy, marshmallows), not land-animal meat directly — do not use feedlot/hormone language. Explain this as a gelatin-sourcing concern instead.]';
      } else if (scenario === 'land_animal') {
        line += '\n    [Meat note: this flag was triggered by land-animal meat or poultry — the feedlot/hormone framing in the system prompt applies directly.]';
      } else {
        // No meatScenario set (e.g. a caller not yet updated) — falls back to
        // the paragraph's own product-name reasoning rather than assuming a scenario.
        line += '\n    [Meat note: check the product name — if it does not clearly indicate land-animal meat, this may be seafood or gelatin; avoid feedlot/hormone language in that case.]';
      }
    }
