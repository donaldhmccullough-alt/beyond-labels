...
### Joel-voice certification-framing audit (July 2026 — FULLY RESOLVED across all three categories: conventional_eggs, conventional_dairy, glyphosate_heavy. PROMPT_VERSION bumped 43 → 44, scan_cache deliberately NOT purged — see note near the end of this entry and "Scan Cache Pattern" above)
...
**Implementation note (conventional_dairy, STAGED — not yet live).** Items 7 and 8 above were reworded
in `pages/api/explain.js` per the wording collaboratively finalized in chat, mirroring the eggs
rework's stance. The Level 1-specific dairy note (`[Level 1 dairy note: ...]`, lines 118-120) was
deliberately left byte-unchanged — its content is tone/emotional-management guidance ("a step to take
when ready... use especially gentle, encouraging language"), not certification-framing, and doesn't
make the near-guarantee claim the rest of this audit is targeting. Item 12 (the real `scan_cache`
example pattern, confirmed across 67 rows, quoted as "before" text) was **not** retroactively edited,
same reasoning as item 11 for eggs — it documents historical shipped output, not live code, and only a
fresh scan after the eventual `PROMPT_VERSION` bump will pick up the new wording. Four new
presence-check tests were added to `__tests__/api/explain.test.js` (describe block "A4.
buildUserMessage() — conventional_dairy note wording"): new wording present, old "the signal that a
farmer chose"/"the meaningful alternative" phrasing absent, no leakage into unrelated categories, and
— the one dairy-specific addition beyond the A3 eggs pattern — a byte-exact check that the Level 1
...
**✅ PROMPT_VERSION BUMPED 43 → 44 — no longer held.** Per the precedent check below, every prior
`SYSTEM_PROMPT` wording change in this project's history has bumped `PROMPT_VERSION` in the same
commit with zero exceptions — this change follows suit. All three categories were fully drafted,
implemented, and tested before the combined bump landed, mirroring the reasoning behind the last
consolidated bump (PROMPT_VERSION 43 — see "Scan Cache Pattern" above). Per explicit instruction,
**`scan_cache` was deliberately NOT purged as part of this bump** — every existing row remains
completely untouched at whatever `prompt_version` it's currently sitting at; only a fresh scan (a real
cache miss) will pick up the new wording for a given `(barcode, user_level)` pair going forward. See
"Scan Cache Pattern" above for the full purge-deferral reasoning, now spanning three consecutive
un-purged bumps (`42`→`43`→`44`).
...
the implementation notes above.

**✅ `PROMPT_VERSION` bumped 43 → 44 — this audit's changes are now live-eligible.** See "Scan Cache
Pattern" above for the full bump note. Per explicit instruction, **`scan_cache` was NOT purged as part
of this bump** — every existing row remains completely untouched at whatever `prompt_version` it's
currently sitting at, for ongoing manual review, the same deliberate choice already made for the 42→43
bump. The three historical-record rows in the table above (11, 12, 13) — real, previously-shipped
`scan_cache` output documenting what the *old* wording produced — will only update on each barcode's
next fresh scan, once that scan naturally misses the cache (which won't happen automatically without a
purge; see "Scan Cache Pattern" for exactly what "not purged" means for existing rows going forward).
The audit itself — the wording decision for all three categories, plus the newly-discovered fourth
surface — is now fully closed, with the code live-eligible pending deploy.
